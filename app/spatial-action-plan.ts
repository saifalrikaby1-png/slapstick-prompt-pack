...
export function buildResolvedSpatialActionPlan(concept: ResolvedProductionConcept, characters: CharacterProfile[]): ResolvedSpatialActionPlan {
  ...
  const objectPath: ResolvedObjectPath = { objectId: "primary-object", objectName: initialObject.objectName, finalZoneId: top.id, segments: [
    { startSeconds: points[0], endSeconds: points[1], fromZoneId: top.id, toZoneId: lower.id, triggerActorId: trapOwner.characterId, triggerAction: "forceful two-hand push", movementType: "roll", direction: { fromZoneId: top.id, toZoneId: lower.id, directionLabel: direction1 }, speedBefore: "stationary", speedAfter: "fast", physicalReason: `${trapOwner.characterName}'s visible push and gravity start the motion` },
  ...
  addBeat(points[3], points[4], [{ actorId: initiator.id, actorName: initiator.shortName, actionType: "settle", sourceZoneId: payoff.id, targetZoneId: payoff.id, forceDescription: "settles unharmed after the backfire" }, { actorId: hero.id, actorName: hero.shortName, actionType: "react", sourceZoneId: lower.id, targetZoneId: lower.id, forceDescription: "remains safe downhill and smiles" }], (chars, objects) => { [initiator.id, assistant?.id].filter(Boolean).forEach((id) => Object.assign(chars.find((state) => state.characterId === id)!, { zoneId: payoff.id, movementState: "settling", posture: "seated in a defeated pose" })); Object.assign(objects[0], { zoneId: top.id, movementState: "settling", direction: undefined, speed: "slow" }); });
...
export function renderModelFacingBeat(beat: SpatialBeatState, context: RenderBeatContext): string {
  const object = context.plan.objectPath.objectName; const actionText = beat.actions.map((action) => {
    if (action.actionType === "push") return `${action.actorName} gives the ${object} one forceful two-paw push, sending it ${action.resultingDirection}.`;
    ...
    if (action.actionType === "tap") return `${action.actorName} makes visible ${action.bodyPartOrContactPoint} contact with the ${object}, absorbs part of its momentum, and redirects it ${action.resultingDirection}.`;
