...
test("inventory classifies zones and attributes separately from movable objects", () => {
  ...
  assert.equal(choreography.classifyPromptEntity("folding screen", "video-prompt", inventory).kind, "movable-object");
...
test("Maximize has phases, updates state and score, persists, rejects worse repair, and supports undo", () => {
  ...
  assert.match(resultsSource, /isRepairImprovement\(initialAnalysis, repairedAnalysis\)/);
