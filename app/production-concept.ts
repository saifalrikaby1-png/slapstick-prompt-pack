...
function buildConcreteConcept(input: ProductionConceptResolutionInput, source: ResolvedProductionConcept["source"]): ResolvedProductionConcept {
  ...
  const response = `${hero?.shortName || "The hero"} sidesteps, plants one foot against the ${object}'s lower side, absorbs part of its momentum with a brief backward lean, then pivots it onto the curved return edge of the authorized path`;
  ...
  const payoff = clean(input.ending) || `${hero?.shortName || "The hero"} stands safely beside the stopped ${object} while ${initiator?.shortName || "the initiator"} and ${assistant?.shortName || "the other character"} sit unharmed together on the authorized ground with surprised defeated expressions`;
  const characters = selected.map((character, index) => ({
    ...
    function: roleFunction(character.role, index),
    openingState: character.id === initiator?.id ? `crouched beside the ${object} at the top of the stone path` : character.id === hero?.id ? `approaching the center of the clearing and watching the ${object}` : `visible beside the path, watching ${initiator?.shortName || "the initiator"}`,
    ...
    endingState: character.id === hero?.id ? `standing safely beside the stopped ${object}` : `seated unharmed on the authorized ground beside the other opposing character, facing ${hero?.shortName || "the hero"}`,
  }));
  const progression = [trigger, response, reversal, payoff];
  const finalComposition = `${location}: ${characters.map((character) => `${character.characterName} ${character.endingState}`).join("; ")}; the ${object} rests on the ground beside ${hero?.shortName || "the hero"}`;
  return {
    ...
    primaryObject: { objectName: object, visualIdentity: `one large, unmistakable ${object} with a bright leaf-shaped mark`, initialPosition: `resting on the ground at the top of the sloped stone path beside ${initiator?.shortName || "the initiator"}`, ownerOrController: initiator?.shortName, forceOrTrigger: trigger, movementPath: `down the sloped stone path toward ${hero?.shortName || "the hero"}; ${hero?.shortName || "the hero"} plants one foot against its lower side, absorbs momentum with a backward lean, and pivots it onto the curved return edge; it rolls back toward ${initiator?.shortName || "the initiator"} before ground friction stops it beside the hero`, interactions: [`passes ${hero?.shortName || "the hero"}`, `${assistant?.shortName || "the assistant"} narrows the path`, `${hero?.shortName || "the hero"} makes visible lower-side foot contact`, `${initiator?.shortName || "the initiator"} dodges into ${assistant?.shortName || "the assistant"}`], finalPosition: `resting on the ground beside ${hero?.shortName || "the hero"} at the center of the clearing` },
