    ...
    <main className="video-production-page">
      ...
      <div className="workspace production-workspace studio-layout">
        ...
        <section className="setup-panel production-card production-selection-panel studio-step-workspace">
          ...
          <section className="form-section scene-setup-workspace" id="production-setup" role="tabpanel" aria-labelledby="workflow-tab-setup" hidden={activeWorkflowTab !== "setup"}>
            <header className="scene-setup-header"><div className="scene-setup-title-group"><span className="scene-setup-title-icon" aria-hidden="true">▣</span><div><h1>Scene Setup</h1><p>Define the environment, key actions, and the overall tone for your video.</p></div></div><button className="scene-how-it-works" type="button" title="Use Scene Setup to align your environment, action, tone, and frame ratios."><span aria-hidden="true">?</span><span>How it works</span></button></header>
            ...
            {productionTab === "core" && <div className="scene-setup-content">
              <section className="scene-panel scene-elements-panel"><h2>Scene Elements</h2><div className="scene-elements-grid">
                <article className="scene-element-card"><div className="scene-element-label"><span aria-hidden="true">⌖</span><span>Location</span></div><input className="scene-input" value={form.location} onChange={(event) => update("location", event.target.value)} placeholder="Choose or describe a location" /><div className="scene-element-image" aria-hidden="true"><span>{form.location || "Location preview"}</span></div></article>
                <article className="scene-element-card"><div className="scene-element-label"><span aria-hidden="true">◇</span><span>Important Object</span></div><input className="scene-input" value={form.importantObject} onChange={(event) => update("importantObject", event.target.value)} placeholder="Choose or describe an object" /><div className="scene-element-image" aria-hidden="true"><span>{form.importantObject || "Object preview"}</span></div></article>
                <article className="scene-element-card"><div className="scene-element-label"><span aria-hidden="true">↗</span><span>Main Action</span></div><input className="scene-input" value={form.trapAction} onChange={(event) => update("trapAction", event.target.value)} placeholder="Describe the action or trap" /><div className="scene-element-image" aria-hidden="true"><span>{form.trapAction || "Action preview"}</span></div></article>
