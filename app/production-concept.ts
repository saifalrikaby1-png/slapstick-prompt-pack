...
function buildConcreteConcept(input: ProductionConceptResolutionInput, source: ResolvedProductionConcept["source"]): ResolvedProductionConcept {
  ...
  const response = `${hero?.shortName || "The hero"} sidesteps at the last moment and redirects the ${object} with a planted foot toward the waiting obstacle`;
  const reversal = `the redirected ${object} rolls back along the same path and knocks the initiator's hiding screen flat`;
  const payoff = clean(input.ending) || `${hero?.shortName || "The hero"} stands safely beside the stopped ${object} while ${others.map((character) => character.shortName).join(" and ") || "the initiator"} sit unharmed behind the flattened screen in one readable defeated pose`;
  const characters = selected.map((character, index) => ({
    ...
    mainAction: character.id === hero?.id ? response : character.id === initiator?.id ? trigger : `points toward the moving ${object} and steps clear of its path`,
    reaction: character.id === hero?.id ? `tracks the ${object}, plants, and redirects it` : `recoils as the ${object} reverses direction`,
    endingState: character.id === hero?.id ? `standing safely beside the stopped ${object}` : `seated unharmed behind the flattened screen, facing ${hero?.shortName || "the hero"}`,
  }));
  ...
  return {
    ...
    primaryObject: { objectName: object, visualIdentity: `one large, unmistakable ${object} with a bright leaf-shaped mark`, initialPosition: `resting on the ground at the top of the sloped stone path beside ${initiator?.shortName || "the initiator"}`, ownerOrController: initiator?.shortName, forceOrTrigger: trigger, movementPath: `down the sloped path toward ${hero?.shortName || "the hero"}, then back along the same path after the redirect`, interactions: [`passes ${hero?.shortName || "the hero"}`, `strikes and flattens the initiator's hiding screen`], finalPosition: `resting on the ground beside ${hero?.shortName || "the hero"} at the center of the clearing` },
