...
- videoLock: metadata and immutable production rules covering platform, model, ratio, duration, style, Creative Direction, motion level, cast, roles, object, setting, narration/voice, continuity, safety, and negative constraints.
...
- When Ultra Retention Mode is enabled, create an immediate first-frame visual hook, a meaningful active change within the first second and 0–3 seconds, one dominant readable event per beat, a physically caused major escalation around 50–65% of duration, and a completed final payoff occupying the last phase. Calibrate the exact movement and camera intensity from Creative Direction.
...
- Apply Creative Direction from frame zero: visual mood, camera style, pacing, performance, music, and sound guide the opening pose and first movement at exactly 0:00, with no neutral introduction.
...
${action === "fix"
    ? `Targeted repair task: return replacements only for the requested fields (${requestedFields.join(", ")}). Read the supplied non-passing Quality Control findings, repair only the responsible requested sections, and leave all unrequested and already-passing sections untouched. Preserve every selected setting, character lock, story fact, platform, model adapter, ratio, duration, Creative Direction, voice layer, audio choice, narration choice, and manually written title. Do not invent a new episode or change user creative choices.`
